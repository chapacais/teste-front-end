<template>
  <Header />
  <section class="menu">
    <Category @open-modal="openModal" v-for="(category, index) in menu" :key="index" :category="category"/>
  </section>
  <Modal 
    ...
    deleteProduct(id) {
      if(confirm('Você tem certeza que quer excluir este produto?')) {
        for(let i = 0; i < this.menu.length; i++) {
          this.menu[i].products = this.menu[i].products.filter((product) => product.id != id);
        }
        this.visible = false;
      }
    },
